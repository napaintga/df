...
* \version 2.20
*
*  This file provides the implementation of Cypress Bootloader SDK.
*
********************************************************************************
* \copyright
...
/* The timeout for Cy_Bootload_Continue(), in milliseconds */
#define BOOTLOAD_DO_TIMEOUT                 (20u)
/* The timeout for the default Cy_Bootload__TransportWrite() call in milliseconds */
...
/* The size in bytes of the bootloader command parameters */
#define PARAMS_SIZE                         (8u) 
/* The length in bytes of data in the "Set App Metadata" bootloader command */
#define DATA_LENGTH                         (9u) 
/* Possible sizes of the data field in the Bootloader packets */
...
#if(CY_BOOTLOAD_APP_FORMAT != CY_BOOTLOAD_BASIC_APP)
...
#endif /* (CY_BOOTLOAD_APP_FORMAT != CY_BOOTLOAD_BASIC_APP) */
...
#if ((CY_BOOTLOAD_OPT_CRYPTO_HW != 0) && (CY_BOOTLOAD_APP_FORMAT == CY_BOOTLOAD_BASIC_APP))
    static bool ComputeSha1(uint32_t address, uint32_t length, uint8_t *result);
#endif /*((CY_BOOTLOAD_OPT_CRYPTO_HW != 0) && (CY_BOOTLOAD_APP_FORMAT == CY_BOOTLOAD_BASIC_APP))*/
...
static cy_en_bootload_status_t VerifyPacket(uint32_t numberRead, const uint8_t packet[]);
static cy_en_bootload_status_t ReadVerifyPacket(uint8_t packet[], bool *noResponse, uint32_t timeout);
static cy_en_bootload_status_t WritePacket(cy_en_bootload_status_t status, uint8_t *packet, uint32_t rspSize);
static void EnterResponse(uint8_t *packet, uint32_t *rspSize, uint32_t *state);

static cy_en_bootload_status_t CopyToDataBuffer(uint8_t dataBuffer[], uint32_t *dataOffset, uint8_t const packet[], 
                                                uint32_t packetSize);

static cy_en_bootload_status_t CommandEnter(uint8_t *packet, uint32_t *rspSize, uint32_t *state, 
                                            cy_stc_bootload_params_t *params);
static cy_en_bootload_status_t CommandProgramData(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params);

#if CY_BOOTLOAD_OPT_ERASE_DATA != 0
static cy_en_bootload_status_t CommandEraseData(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params);
#endif
    
#if CY_BOOTLOAD_OPT_VERIFY_DATA != 0
static cy_en_bootload_status_t CommandVerifyData(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params);
#endif /* CY_BOOTLOAD_OPT_VERIFY_DATA != 0*/

#if CY_BOOTLOAD_OPT_SEND_DATA != 0
static cy_en_bootload_status_t CommandSendData(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params);
#endif /* CY_BOOTLOAD_OPT_SEND_DATA != 0 */

#if CY_BOOTLOAD_OPT_VERIFY_APP != 0
static cy_en_bootload_status_t CommandVerifyApp(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params);
#endif /* CY_BOOTLOAD_OPT_VERIFY_APP != 0 */

static cy_en_bootload_status_t CommandSetAppMetadata(uint8_t *packet, uint32_t *rspSize, 
                                                     cy_stc_bootload_params_t *params);

#if CY_BOOTLOAD_OPT_GET_METADATA != 0
static cy_en_bootload_status_t CommandGetMetadata(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params);
#endif /* CY_BOOTLOAD_OPT_GET_METADATA != 0 */

#if CY_BOOTLOAD_OPT_SET_EIVECTOR != 0
    static cy_en_bootload_status_t CommandSetEIVector(uint8_t *packet, uint32_t *rspSize,
                                                      cy_stc_bootload_params_t *params);
#endif /* CY_BOOTLOAD_OPT_SET_EIVECTOR != 0 */

static cy_en_bootload_status_t CommandUnsupported(uint8_t *packet, uint32_t *rspSize, 
                                                              cy_stc_bootload_params_t *params );
static cy_en_bootload_status_t ContinueHelper(uint32_t bootloaderCommand, uint8_t *packet, uint32_t *rspSize, 
                                              cy_stc_bootload_params_t *params, bool *noResponse);
#if(CY_BOOTLOAD_APP_FORMAT != CY_BOOTLOAD_BASIC_APP)
    #if(CY_BOOTLOAD_SEC_APP_VERIFY_TYPE == CY_BOOTLOAD_VERIFY_FAST)
    ...
    #endif /* CY_BOOTLOAD_SEC_APP_VERIFY_TYPE == CY_BOOTLOAD_VERIFY_FAST */
    bool VerifySecureApp(uint32_t verifyStartAddr, uint32_t verifyLength, uint32_t signatureAddr);
#endif/*(CY_BOOTLOAD_APP_FORMAT != CY_BOOTLOAD_BASIC_APP)*/


/*******************************************************************************
* Function Name: Cy_Bootload_DoBootload
****************************************************************************//**
*
* This function starts the application download and install operations.
* It calls Cy_Bootload_Init() and then continually calls Cy_Bootload_Continue().
* It does not return until the bootload operation is complete or an error
* is detected.
* This function provides an example of how to use the Bootloader SDK to quickly
* design a bootload system. \note This function blocks, suspending all other
* tasks until the bootload operation is complete. To execute other tasks
* while bootloading, design your code to call Cy_Bootload_Init() and
* Cy_Bootload_Continue() directly.
* Only one bootloading operation can be done at a time - the user's code must 
* ensure this. 
...
*
* \param state      The pointer to a bootloading state variable.
*                   See \ref group_bootload_macro_state.
* \param timeout    The timeout in milliseconds for bootloading.
*
* \return See \ref cy_en_bootload_status_t.
*
*******************************************************************************/
cy_en_bootload_status_t  Cy_Bootload_DoBootload(uint32_t *state, uint32_t timeout)
{   
    /* The Bootloader parameters, used to configure a bootloader */
    cy_stc_bootload_params_t params;
    
    /* The status code for the Bootloader SDK API */
    cy_en_bootload_status_t status;
    
    /*
    * Used to count seconds, to convert counts to seconds use
    * counterTimeoutSeconds(SECONDS, BOOTLOAD_DO_TIMEOUT)
    */
    uint32_t count = 0u;
    bool finished = false; /* To exit do {} while () loop */
    
    /* The buffer to save bootloader commands to */
    CY_ALIGN(4) uint8_t buffer[CY_BOOTLOAD_SIZEOF_DATA_BUFFER];
    /* The buffer for packets sent and received with the Transport API */
    CY_ALIGN(4) uint8_t packet[CY_BOOTLOAD_SIZEOF_CMD_BUFFER ];
    ...
    status = Cy_Bootload_Init(state, &params);
    ...
    /* Run the Bootloader */
    params.timeout = BOOTLOAD_DO_TIMEOUT;
    Cy_Bootload_TransportStart();

    do
    {
        
        status = Cy_Bootload_Continue(state, &params);
        if (*state == CY_BOOTLOAD_STATE_FINISHED)
        {
            /* The bootloading of the application image is finished, the image is valid */
            finished = true;
        }
        else if (*state == CY_BOOTLOAD_STATE_FAILED)
        {
            /* The bootloading has failed */
            finished = true;
        }
        ...
        /* No bootloader command received during the timeout period */
        if ( ( (count * BOOTLOAD_DO_TIMEOUT) >= timeout)
          && (*state == CY_BOOTLOAD_STATE_NONE   )  )
        {
            finished = true;
        }
    }
...
/*******************************************************************************
* Function Name: Cy_Bootload_Init
****************************************************************************//**
*
* This function starts the application download and install operations.
* Make subsequent calls to Cy_Bootload_Continue() to continue the
* process. \n
...
* \param state      The pointer to a bootloading state variable
*                   See \ref group_bootload_macro_state 
* \param params     The pointer to a bootloader parameters structure
*                   See \ref cy_stc_bootload_params_t 
*
* \return See \ref cy_en_bootload_status_t.
* - \ref CY_BOOTLOAD_SUCCESS if successful.
* - \ref CY_BOOTLOAD_ERROR_UNKNOWN if a NULL pointer is passed.
* 
*******************************************************************************/
cy_en_bootload_status_t Cy_Bootload_Init(uint32_t *state, cy_stc_bootload_params_t *params)
{
    ...
    {
        status = CY_BOOTLOAD_ERROR_UNKNOWN;
    }
...
/*******************************************************************************
* Function Name: Cy_Bootload_ExecuteApp
****************************************************************************//**
*
...
void Cy_Bootload_ExecuteApp(uint32_t appId)
{
    ...
    cy_bootload_appId = (uint8_t)appId;
    CySoftwareReset();
}
...
/*******************************************************************************
* Function Name: Cy_Bootload_SwitchToApp
****************************************************************************//**
*
...
* \param appId      An application number of the application to switch to.
*
* \return If failed, returns the status code. See \ref cy_en_bootload_status_t.
* 
*******************************************************************************/
cy_en_bootload_status_t Cy_Bootload_SwitchToApp(uint32_t appId)
{
    ...
    status = Cy_Bootload_GetAppMetadata(appId, &startAddress, NULL);
    if (status == CY_BOOTLOAD_SUCCESS)
    {
    ...
    #elif(CY_BOOTLOAD_APP_FORMAT == CY_BOOTLOAD_CYPRESS_APP)
...
/*******************************************************************************
* Function Name: Cy_Bootload_GetAppMetadata
****************************************************************************//**
*
...
* 
*******************************************************************************/
__WEAK cy_en_bootload_status_t Cy_Bootload_GetAppMetadata(uint32_t appId, uint32_t *verifyAddress, uint32_t *verifySize)
{
    cy_en_bootload_status_t status = CY_BOOTLOAD_SUCCESS; 
...
* and \ref CY_BOOTLOAD_OPT_CRYPTO_HW is set.
* 
...
* 
* \return See \ref cy_en_bootload_status_t.
* - \ref CY_BOOTLOAD_SUCCESS If the application is valid.
* - \ref CY_BOOTLOAD_ERROR_VERIFY If the application is invalid.
* 
*******************************************************************************/
__WEAK cy_en_bootload_status_t Cy_Bootload_ValidateApp(uint32_t appId, cy_stc_bootload_params_t *params)
{
    ...
    cy_en_bootload_status_t status = Cy_Bootload_GetAppMetadata(appId, &appVerifyStartAddress, &appVerifySize);
    

    if (status == CY_BOOTLOAD_SUCCESS)
    {
    #if(CY_BOOTLOAD_APP_FORMAT == CY_BOOTLOAD_CYPRESS_APP)
    ...
    #elif (CY_BOOTLOAD_APP_FORMAT == CY_BOOTLOAD_SIMPLIFIED_APP)
        status = (VerifySecureApp(appVerifyStartAddress, appVerifySize, appVerifyStartAddress - RSA_CHECKSUM_LENGTH))?
            ...
            { 
                status = (memcmp((const void *)sha1buf, (const void *)appFooterAddress, SHA1_CHECKSUM_LENGTH) == 0UL)? 
                                                                        CY_BOOTLOAD_SUCCESS : CY_BOOTLOAD_ERROR_VERIFY;
            }
            else
            {
                status = CY_BOOTLOAD_ERROR_VERIFY;
            }
        #else

            uint32_t appCrc = Cy_Bootload_DataChecksum((uint8_t *)appVerifyStartAddress, appVerifySize, params);
            uint32_t appFooterAddress = (appVerifyStartAddress + appVerifySize);
            status = (*(uint32_t*)appFooterAddress == appCrc) ? CY_BOOTLOAD_SUCCESS : CY_BOOTLOAD_ERROR_VERIFY;
        #endif/* (CY_BOOTLOAD_OPT_CRYPTO_HW != 0) */
    #endif /* (CY_BOOTLOAD_APP_FORMAT == CY_BOOTLOAD_CYPRESS_APP) */
    }
    return (status);
}
...
/*******************************************************************************
* Function Name: Cy_Bootload_GetRunningApp
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: Cy_Bootload_CopyApp
****************************************************************************//**
*
                                            ...
                                            uint32_t rowSize, cy_stc_bootload_params_t *params)
{
    ...
    {
        status = Cy_Bootload_ReadData(readAddr, rowSize, CY_BOOTLOAD_IOCTL_READ, params);
        if (status == CY_BOOTLOAD_SUCCESS)
        {
            status = Cy_Bootload_WriteData(writeAddr, rowSize, CY_BOOTLOAD_IOCTL_WRITE, params);
        }
        if (status != CY_BOOTLOAD_SUCCESS)
        {
            break;
...
/*******************************************************************************
* Function Name: Cy_Bootload_OnResetApp0
****************************************************************************//**
*
...
{
    /* Set cy_bootload_appId to ZERO under a non-software reset. This means
     * that the Bootloader application is scheduled - the initial clean state. 
     * The value of cy_bootload_appId is valid only under a software reset.
     */    
    if (Cy_SysLib_GetResetReason() != CY_SYSLIB_RESET_SOFT)
    {
        cy_bootload_appId = 0u;
    }
    else
    {
        if (cy_bootload_appId != 0u)
        {
            (void) Cy_Bootload_SwitchToApp((uint32_t) cy_bootload_appId);
        }
    }
}
...
* - CY_BOOTLOAD_IOCTL_READ    - Only read.
* - CY_BOOTLOAD_IOCTL_COMPARE - Compare the data in the buffer with the data in 
*                               memory.
* - CY_BOOTLOAD_IOCTL_BHP     - Decrypt data before comparing the buffer with 
*                               memory,
*   if the bootloader Host provided encrypted data.
* \param params     The pointer to a bootloader parameters structure.
*                   See \ref cy_stc_bootload_params_t .
* \return See \ref cy_en_bootload_status_t
* - CY_BOOTLOAD_SUCCESS - If successful.
* - CY_BOOTLOAD_ERROR_LENGTH if \c The length value is invalid.
* - CY_BOOTLOAD_ERROR_ADDRESS if \c The address is invalid.
* 
*******************************************************************************/
__WEAK cy_en_bootload_status_t Cy_Bootload_ReadData (uint32_t address, uint32_t length, uint32_t ctl, 
                                                     cy_stc_bootload_params_t *params)
{
    /*
...
* - CY_BOOTLOAD_IOCTL_WRITE - Only write.
* - CY_BOOTLOAD_IOCTL_ERASE - Erase the sector, the sector size can be bigger 
*   than the size of the page to write.
* - CY_BOOTLOAD_IOCTL_BHP   - Decrypt data before writing to memory, if 
*   the Bootloader Host provided encrypted data.
* \param params     The pointer to a bootloader parameters structure.
*                   See \ref cy_stc_bootload_params_t .
* \return See \ref cy_en_bootload_status_t.
* - CY_BOOTLOAD_SUCCESS - If successful.
...
* 
*******************************************************************************/
__WEAK cy_en_bootload_status_t Cy_Bootload_WriteData(uint32_t address, uint32_t length, uint32_t ctl, 
                                                     cy_stc_bootload_params_t *params)
{
    /*
    * This function does nothing, weak implementation.
...
* This function must be implemented in the user's code.
* 
* This function receives a command packet from the Bootloader Host via the 
...
* - CY_BOOTLOAD_SUCCESS - If successful.
* - CY_BOOTLOAD_ERROR_TIMEOUT - If no data is received.
* - See \ref cy_en_bootload_status_t.
*
*******************************************************************************/
__WEAK cy_en_bootload_status_t Cy_Bootload_TransportRead(uint8_t buffer[], uint32_t size, uint32_t *count, 
                                                         uint32_t timeout)
{
    /*
    * This function does nothing, weak implementation.
...
* This function must be implemented in the user's code.
*
* This function transmits a response packet to the bootloader host via the 
* communication channel. The function waits for a timeout until all bytes are
* sent.
*
...
* - CY_BOOTLOAD_SUCCESS - If successful.
* - CY_BOOTLOAD_ERROR_TIMEOUT - If no data is transmitted.
* 
*******************************************************************************/
__WEAK cy_en_bootload_status_t Cy_Bootload_TransportWrite(uint8_t buffer[], uint32_t size, uint32_t *count, 
                                                          uint32_t timeout)
{
    /*
    * This function does nothing, weak implementation.
...
*
*******************************************************************************/
__WEAK void Cy_Bootload_TransportReset(void)
{
    /*
    * This function does nothing, weak implementation.
    * The purpose of this code is to disable compiler warnings for Non-optimized
...
*
* This function must be implemented in the user's code. \n
* Starts the communication interface through which bootloading will be working.
*
*******************************************************************************/
__WEAK void Cy_Bootload_TransportStart(void)
{
    /*
    * This function does nothing, weak implementation.
    * The purpose of this code is to disable compiler warnings for Non-optimized
...
*
* This function must be implemented in the user's code. \n
* Stops the communication interface through which bootloading will be working.
*
*******************************************************************************/
__WEAK void Cy_Bootload_TransportStop(void)
{
    /*
    * This function does nothing, weak implementation.
    * The purpose of this code is to disable compiler warnings for Non-optimized
...
/*******************************************************************************
*        Cy_Bootload_Continue related code, till the EOF
*******************************************************************************/
...
*
* \param size The bootloader packet size value.
*
...
****************************************************************************//**
*
* This function returns a bootloader packet command value for a given \c packet.
*
* \param packet  The pointer to the byte array containing bootloader packet
*                data.
*
* \return A bootloader packet command value.
* 
*******************************************************************************/
static uint32_t GetPacketCommand(const uint8_t packet[])
{
...
****************************************************************************//**
*
* This function returns a data size in the bootloader packet. \n
...
****************************************************************************//**
*
* This function is used to validate a bootloader packet footer.
*
* \param packet      The pointer to a byte array containing the 
*                    bootloader packet data.
* \param packetSize  The offset within the data bytes in the packet. \n
*                    E.g. 0 means the first byte of the data.
* \return
* - 0, if the bootloader packet footer is invalid.
* - 1, if the bootloader packet footer is valid.
*
*******************************************************************************/
static uint32_t ValidatePacketFooter(const uint8_t packet[], uint32_t packetSize)
{
...
****************************************************************************//**
*
* This function is used to set a bootloader packet header value.
*
* \param packet     The pointer to a byte array containing the bootloader packet
*                   data.
*
*******************************************************************************/
static void SetPacketHeader(uint8_t packet[])
...
****************************************************************************//**
*
* This function is used to set a bootloader packet command value.
*
* \param packet     The pointer to a byte array containing the
*                   bootloader packet data.
* \param cmd        The command value to be set in the bootloader packet.
*
*******************************************************************************/
static void SetPacketCmd(uint8_t packet[], uint32_t cmd)
{
...
****************************************************************************//**
*
* This function is used to set a bootloader packet size value.
*
* \param packet     The pointer to a byte array containing the 
*                   bootloader packet data.
* \param size       The value for the bootloader packet size.
*
*******************************************************************************/
static void SetPacketDSize(uint8_t packet[], uint32_t size)
{
...
****************************************************************************//**
*
* This function is used to set a bootloader packet checksum value.
*
* \param packet     The pointer to a byte array containing the 
*                   bootloader packet data.
* \param size       The bootloader packet size value.
* \param checksum   The value for a bootloader packet checksum.
*
*******************************************************************************/
static void SetPacketChecksum(uint8_t packet[], uint32_t size, uint32_t checksum)
{
...
****************************************************************************//**
*
* This function is used to set a bootloader packet footer value.
*
* \param packet     The pointer to a byte array containing the
*                   bootloader packet data.
* \param size       The bootloader packet size value.
...
/*******************************************************************************
* Function Name: Cy_Bootload_ValidateMetadata
****************************************************************************//**
*
* The function checks if the bootloader metadata is valid.
*
* \return See \ref cy_en_bootload_status_t 
* - \ref CY_BOOTLOAD_SUCCESS - If metadata is valid.
* - \ref CY_BOOTLOAD_ERROR_VERIFY - If metadata is not valid.
* 
*******************************************************************************/
cy_en_bootload_status_t Cy_Bootload_ValidateMetadata(uint32_t metadataAddress, cy_stc_bootload_params_t *params)
{
    ...
    uint32_t crc = Cy_Bootload_DataChecksum( (uint8_t *)metadataAddress, metadataLength - CRC_CHECKSUM_LENGTH, params);
    uint32_t crcMeta = *(uint32_t *)(metadataAddress + (metadataLength - CRC_CHECKSUM_LENGTH) );
    cy_en_bootload_status_t status = (crc == crcMeta) ? CY_BOOTLOAD_SUCCESS : CY_BOOTLOAD_ERROR_VERIFY;
    return (status);
}
...
* This function is used to calculate the checksum of bootloader packets.
* 
* MISRA requires this function have no side effects.
*
...
static uint32_t PacketChecksum(const uint8_t buffer[], uint32_t size)
{
#if (CY_BOOTLOAD_OPT_PACKET_CRC != 0u)
    uint16_t crc = CRC_CCITT_INIT;
    uint16_t tmp;
    uint32_t i;
    uint16_t tmpIndex;
    
    size += PACKET_DATA_IDX; /* 4 bytes before data in Cypress bootloader packet */
...
#endif /* CY_BOOTLOAD_OPT_PACKET_CRC != 0u */
}


/*******************************************************************************
* Function Name: Cy_Bootload_DataChecksum
****************************************************************************//**
*
* This function computes a CRC-32C for the provided number of bytes contained
* in the provided buffer. \n
* This function is used to validate the Program Data and Verify Data bootloader
* commands and a metadata row.
* \note Ensure the Crypto block is properly initialized
* if \ref CY_BOOTLOAD_OPT_CRYPTO_HW is set.
* 
...
{
#if CY_BOOTLOAD_OPT_CRYPTO_HW != 0 /* Use PDL Hardware Crypto API */
...
****************************************************************************//**
*
* This function is used inside Bootloader_ReadVerifyPacket() to verify if a
* received packet is correct.
*
* \param numberRead     The number of bytes read from the communication
*                       interface.
* \param packet         The pointer to the bootloader packet buffer.
*
* \return  See \ref cy_en_bootload_status_t.
*
*******************************************************************************/
static cy_en_bootload_status_t VerifyPacket(uint32_t numberRead, const uint8_t packet[])
{
    ...
    if ((numberRead < CY_BOOTLOAD_PACKET_MIN_SIZE) || (packet[PACKET_SOP_IDX] != PACKET_SOP_VALUE))
    {
    ...
    else
    {
         ...
         */
        if (   ((packetSize + CY_BOOTLOAD_PACKET_MIN_SIZE) > numberRead)
            || ((packetSize + CY_BOOTLOAD_PACKET_MIN_SIZE) > CY_BOOTLOAD_SIZEOF_CMD_BUFFER)  )
        {
        ...
        else /* The packet length is OK */
        {
            ...
            else
            {
                ...
                {
                    status = CY_BOOTLOAD_ERROR_CHECKSUM;
                }
            }
        }
    }
...
****************************************************************************//**
*
* This function is used inside Cy_Bootload_Continue to read and verify
* a received bootloader packet.
*
* \param packet       The pointer to the bootloader packet buffer.
* \param noResponse   The pointer to a variable that states whether to send
*                      a response back to a Bootloader Host or not.
* \param timeout      The timeout in milliseconds to wait.
*
* \return See \ref cy_en_bootload_status_t
* - \ref CY_BOOTLOAD_SUCCESS - If a packet is successfully received.
* - \ref CY_BOOTLOAD_ERROR_TIMEOUT - If no packet is received during
*   the timeout period.
* 
*******************************************************************************/
static cy_en_bootload_status_t ReadVerifyPacket(uint8_t packet[], bool *noResponse, uint32_t timeout)
{
    ...
    status = Cy_Bootload_TransportRead( packet, CY_BOOTLOAD_SIZEOF_CMD_BUFFER, &numberRead, timeout );

    if (status == CY_BOOTLOAD_ERROR_TIMEOUT)
    {
        *noResponse = true;
    }
    if (status == CY_BOOTLOAD_SUCCESS)
...
****************************************************************************//**
*
* This function creates a bootloader response packet and transmits it back to 
* the bootloader host application over the already established communications
* protocol.
*
* \param status     The error response code of the bootloader response packet.
* \param packet     The pointer to the bootloader packet buffer.
* \param rspSize    The number of bytes contained within the \c packet
*                   to pass back to a Bootloader Host.
*
* \return See \ref cy_en_bootload_status_t
* 
*******************************************************************************/
static cy_en_bootload_status_t WritePacket(cy_en_bootload_status_t status, uint8_t *packet, uint32_t rspSize)
{
    uint32_t checksum;
    /*
    * The Bootloader Host expects only one byte of the status,
    ...
    */
    ...
    return ( Cy_Bootload_TransportWrite(packet, rspSize + CY_BOOTLOAD_PACKET_MIN_SIZE, &rspSize, 
...
****************************************************************************//**
*
* This function is used inside Cy_Bootload_ContinueEntr() to add version and
* size fields.
*
* \param packet     The pointer to the bootloader packet buffer.
...
static void EnterResponse(uint8_t *packet, uint32_t *rspSize, uint32_t *state)
{
    static const cy_stc_bootload_enter_t bootloaderVersion =
    {
        CY_BOOTLOAD_SILICON_ID,
        (uint8_t) CY_BOOTLOAD_SILICON_REV,
        {
            (uint8_t) CY_BOOTLOADER_SDK_VERSION_MINOR,
            (uint8_t) CY_BOOTLOADER_SDK_VERSION_MAJOR,
            0x01u /* Used for BWC with the Bootloader component */
        }
    };
    *state = CY_BOOTLOAD_STATE_BOOTLOADING;
    *rspSize = sizeof(bootloaderVersion);
    (void) memcpy( GetPacketData(packet, PACKET_DATA_NO_OFFSET), &bootloaderVersion, *rspSize);
}
...
****************************************************************************//**
*
* This is a helper function for Cy_Bootload_Continue().
*
* This function is called to handle Command Enter, see the Bootloader Commands
* section in the Bootloader SDK User's Guide.
*
* \param packet     The pointer to the bootloader packet buffer.
* \param rspSize    The pointer to a response packet size.
* \param state      The pointer to a bootloading state variable.
...
static cy_en_bootload_status_t CommandEnter(uint8_t *packet, uint32_t *rspSize, uint32_t *state, 
                                            cy_stc_bootload_params_t *params)
{
    cy_en_bootload_status_t status = CY_BOOTLOAD_ERROR_LENGTH;
    volatile uint32_t productId = ElfSymbolToAddr(&__cy_product_id);
    uint32_t packetSize = GetPacketDSize(packet);
    *rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    
    if (packetSize == 0u)  /* 'Product ID' not demanded */
    {
        status = (productId == 0u)? CY_BOOTLOAD_SUCCESS : CY_BOOTLOAD_ERROR_LENGTH;
        if (status == CY_BOOTLOAD_SUCCESS)
        {
...
*                       dataBuffer length.
* \param packet         The pointer to the bootloader packet buffer.
* \param packetSize     The length in bytes of the data in the bootloader
*                       packet buffer.
*
* \return See \ref cy_en_bootload_status_t
* 
                                                ...
                                                uint32_t packetSize)
{
    ...
    {
        status = CY_BOOTLOAD_ERROR_LENGTH;
        if ( (*dataOffset + packetSize) <= CY_BOOTLOAD_SIZEOF_DATA_BUFFER )
        {
            status = CY_BOOTLOAD_SUCCESS;
...
****************************************************************************//**
*
* This a helper function for Cy_Bootload_Continue().
...
{   
    ...
    *rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    if (packetSize >= PARAMS_SIZE)
    {
        ...
        /* Data may be sent with the Program Data bootloader command, so copy it to dataBuffer */
        status = CopyToDataBuffer(dataBufferLocal, dataOffsetLocal, GetPacketData(packet, PARAMS_SIZE),
                                  packetSize - PARAMS_SIZE );
        
        if (status == CY_BOOTLOAD_SUCCESS)
        {
            if (crc != Cy_Bootload_DataChecksum(dataBufferLocal, *dataOffsetLocal, params) )
            {
                status = CY_BOOTLOAD_ERROR_CHECKSUM;
            }
        }
        if (status == CY_BOOTLOAD_SUCCESS)
        {
            status = Cy_Bootload_WriteData(address, *dataOffsetLocal, CY_BOOTLOAD_IOCTL_BHP, params);
        }
        if (status == CY_BOOTLOAD_SUCCESS)
        {
            status = Cy_Bootload_ReadData (address, *dataOffsetLocal, CY_BOOTLOAD_IOCTL_COMPARE, params);
        }
    } /* if (packetSize >= PARAMS_SIZE) */
    *dataOffsetLocal = 0u;
    return (status);
...
static cy_en_bootload_status_t CommandEraseData(uint8_t  *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params)
{
    ...
    *rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    ...
    {
        uint32_t address = GetU32( GetPacketData(packet, PACKET_DATA_NO_OFFSET) );
        status = Cy_Bootload_WriteData(address, 0u, CY_BOOTLOAD_IOCTL_ERASE, params);
    }
    params->dataOffset = 0u;
    return (status);
}
#endif /* CY_BOOTLOAD_OPT_ERASE_DATA != 0 */
...
static cy_en_bootload_status_t CommandVerifyData(uint8_t  *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params)
{
    ...
    *rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    if (packetSize >= PARAMS_SIZE)
    {
        ...
        {
            status = Cy_Bootload_ReadData(address, *dataOffsetLocal, CY_BOOTLOAD_IOCTL_COMPARE, params);
            status = (status == CY_BOOTLOAD_SUCCESS) ? CY_BOOTLOAD_SUCCESS : CY_BOOTLOAD_ERROR_VERIFY;
        }
    }
    *dataOffsetLocal = 0u;
    return (status);
...
*
* This is a helper function for Cy_Bootload_Continue().
* This function handles the Send Data bootloader command.
*
* \param packet     The pointer to the bootloader packet buffer.
* \param rspSize    The pointer to a response packet size.
* \param params     The pointer to a bootloader parameters structure.
...
*
* This is a helper function for Cy_Bootload_Continue().
* This function handles the Verify Application bootloader command.
*
* \param packet     The pointer to the bootloader packet buffer.
* \param rspSize    The pointer to a response packet size.
* \param params     The pointer to a bootloader parameters structure.
...
static cy_en_bootload_status_t CommandVerifyApp(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params)
{
    ...
    *rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    ...
    if ( (status == CY_BOOTLOAD_SUCCESS) || (status == CY_BOOTLOAD_ERROR_VERIFY) )
    {
        uint8_t *valid = GetPacketData(packet, PACKET_DATA_NO_OFFSET);
        *valid = (status == CY_BOOTLOAD_SUCCESS) ? 1u : 0u;
        status = CY_BOOTLOAD_SUCCESS;
        *rspSize = CY_BOOTLOAD_RSP_SIZE_VERIFY_APP;
    }
    return (status);
}
#endif /* CY_BOOTLOAD_OPT_VERIFY_APP != 0 */
...
#if CY_BOOTLOAD_METADATA_WRITABLE != 0
/*******************************************************************************
* Function Name: Cy_Bootload_SetAppMetadata
****************************************************************************//**
*
* This is a helper function for Cy_Bootload_Continue().
...
* 
*******************************************************************************/
cy_en_bootload_status_t Cy_Bootload_SetAppMetadata(uint32_t appId, uint32_t verifyAddress, uint32_t verifySize, 
                                                   cy_stc_bootload_params_t *params)
{
    cy_en_bootload_status_t status = CY_BOOTLOAD_SUCCESS;
    uint32_t metadataAddress = 0u;
    uint32_t metadataLength  = 0u;
    if ((params == NULL) || (appId >= CY_BOOTLOAD_MAX_APPS))
    {
        status = CY_BOOTLOAD_ERROR_UNKNOWN;
    }
    if (status == CY_BOOTLOAD_SUCCESS)
    {
        ...
        status = Cy_Bootload_ReadData(metadataAddress, metadataLength, CY_BOOTLOAD_IOCTL_READ, params);
    }
    if (status == CY_BOOTLOAD_SUCCESS)
    {    
        ...
        if ( (getVerifyAddress != verifyAddress) || ( getVerifySize != verifySize) )
        {
            ...
            crc = Cy_Bootload_DataChecksum(params->dataBuffer, metadataLength - CRC_CHECKSUM_LENGTH, params);
            PutU32(params->dataBuffer, metadataLength - CRC_CHECKSUM_LENGTH, crc);
            status = Cy_Bootload_WriteData(metadataAddress, metadataLength, CY_BOOTLOAD_IOCTL_WRITE, params);
        }
    }
    return (status);
}
...
****************************************************************************//**
* This is a helper function for Cy_Bootload_Continue().
* This function handles the Set Application Metadata bootloader command.
*
* \param packet     The pointer to the bootloader packet buffer.
* \param rspSize    The pointer to a response packet size.
* \param params     The pointer to a bootloader parameters structure.
                                                     ...
                                                     cy_stc_bootload_params_t *params)
{
    ...
    *rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    ...
    if (status == CY_BOOTLOAD_SUCCESS)
    {
        /* Data offsets 0, 1, 5 are defined in the Bootloader Packet Structure */
        ...
        status = Cy_Bootload_SetAppMetadata(app, verifyAddress, verifySize, params);
    #endif
...
****************************************************************************//**
* This is a helper function for Cy_Bootload_Continue().
* This function handles the Get Metadata bootloader command.
*
* \param packet     The pointer to the bootloader packet buffer.
* \param rspSize    The pointer to a response packet size.
* \param params     The pointer to a bootloader parameters structure.
...
{
    cy_en_bootload_status_t status = CY_BOOTLOAD_SUCCESS;
    uint32_t locRspSize = CY_BOOTLOAD_RSP_SIZE_0;
    ...
    if (status == CY_BOOTLOAD_SUCCESS)
    {
        ...
        if ( (toAddr < fromAddr)
          || ( ( (toAddr - fromAddr) + CY_BOOTLOAD_PACKET_MIN_SIZE) > CY_BOOTLOAD_SIZEOF_CMD_BUFFER) )
        {
            status  = CY_BOOTLOAD_ERROR_DATA;
        }
        else
        {
            ...
            status = Cy_Bootload_ValidateMetadata(metadataAddr, params);
            if (status == CY_BOOTLOAD_SUCCESS)
            {
                status = Cy_Bootload_ReadData(metadataAddr, metadataLength, CY_BOOTLOAD_IOCTL_READ, params);
            }
            if (status == CY_BOOTLOAD_SUCCESS)
            {
...
*
* This is a helper function for Cy_Bootload_Continue().
* This function copies the Encryption Initial Vector value from the bootloader 
...
* 
*******************************************************************************/
static cy_en_bootload_status_t CommandSetEIVector(uint8_t *packet, uint32_t *rspSize, cy_stc_bootload_params_t *params)
{
    cy_en_bootload_status_t status = CY_BOOTLOAD_SUCCESS;
    *rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    ...
    {
        memcpy(params->encryptionVector, GetPacketData(packet, PACKET_DATA_NO_OFFSET), size);
    }
    else
    {
        status = CY_BOOTLOAD_ERROR_DATA;
...
* 
*******************************************************************************/
static cy_en_bootload_status_t CommandUnsupported(uint8_t packet[], uint32_t *rspSize, cy_stc_bootload_params_t *params)
{
    cy_en_bootload_status_t status = CY_BOOTLOAD_ERROR_CMD;
...
****************************************************************************//**
* This is a helper function for Cy_Bootload_Continue(). This function parses
*  most of the bootloader commands.
*
* \param packet     The pointer to the bootloader packet buffer.
* \param rspSize    The pointer to a response packet size.
* \param params     The pointer to a bootloader parameters structure.
                                              ...
                                              cy_stc_bootload_params_t *params, bool *noResponse)
{
    ...
    {
    case CY_BOOTLOAD_CMD_PROGRAM_DATA:
        status = CommandProgramData(packet, rspSize, params);
        break;

#if CY_BOOTLOAD_OPT_VERIFY_DATA != 0
    case CY_BOOTLOAD_CMD_VERIFY_DATA:
        status = CommandVerifyData(packet, rspSize, params);
        break;
#endif /* CY_BOOTLOAD_OPT_VERIFY_DATA != 0 */

#if CY_BOOTLOAD_OPT_ERASE_DATA != 0
    case CY_BOOTLOAD_CMD_ERASE_DATA:
        status = CommandEraseData(packet, rspSize, params);
        break;
#endif /* CY_BOOTLOAD_OPT_ERASE_DATA != 0 */

#if CY_BOOTLOAD_OPT_VERIFY_APP != 0
    case CY_BOOTLOAD_CMD_VERIFY_APP:
        status = CommandVerifyApp(packet, rspSize, params);
        break;
#endif /* CY_BOOTLOAD_OPT_VERIFY_APP != 0 */

#if CY_BOOTLOAD_OPT_SEND_DATA != 0
    case CY_BOOTLOAD_CMD_SEND_DATA_WR:
        *noResponse = true;
        status = CommandSendData(packet, rspSize, params);
        break;
        
    case CY_BOOTLOAD_CMD_SEND_DATA:
        status = CommandSendData(packet, rspSize, params);
        break;
#endif /* CY_BOOTLOAD_NO_CMD_SEND_DATA == 0 */

    case CY_BOOTLOAD_CMD_SYNC: /* If something fails, then the Host sends this command to reset the Bootloader */
        params->dataOffset = 0u;
        *noResponse = true;
        break;

    case CY_BOOTLOAD_CMD_SET_APP_META:
        status = CommandSetAppMetadata(packet, rspSize, params);
        break;

#if CY_BOOTLOAD_OPT_GET_METADATA != 0
    case CY_BOOTLOAD_CMD_GET_METADATA:
        status = CommandGetMetadata(packet, rspSize, params);
        break;
#endif /* CY_BOOTLOAD_OPT_GET_METADATA != 0 */

#if CY_BOOTLOAD_OPT_SET_EIVECTOR != 0
    case CY_BOOTLOAD_CMD_SET_EIVECTOR:
...
/*******************************************************************************
* Function Name: Cy_Bootload_Continue
****************************************************************************//**
*
* This function causes the Bootloader to attempt to read data being transmitted 
...
{
    cy_en_bootload_status_t status = CY_BOOTLOAD_ERROR_UNKNOWN; /* Give a value to a close warning */
    uint8_t *packet = params->packetBuffer; /* Receive/Transmit buffer */  

    uint32_t rspSize = CY_BOOTLOAD_RSP_SIZE_0;
    ...
    if ( (*state == CY_BOOTLOAD_STATE_NONE) || (*state == CY_BOOTLOAD_STATE_BOOTLOADING) )
    {
        status = ReadVerifyPacket(packet, &noResponse, params->timeout);
        if (status == CY_BOOTLOAD_SUCCESS)
        {
            uint32_t bootloaderCommand = GetPacketCommand(packet);

            if      (bootloaderCommand == CY_BOOTLOAD_CMD_ENTER)
            {
                status = CommandEnter(packet, &rspSize, state, params);
            }
            else if (bootloaderCommand == CY_BOOTLOAD_CMD_EXIT)
            {
                *state = CY_BOOTLOAD_STATE_FINISHED;
                noResponse = true;
            }
            else if (*state != CY_BOOTLOAD_STATE_BOOTLOADING)
            {
                status = CY_BOOTLOAD_ERROR_CMD;
            }
            else
            {
                status = ContinueHelper(bootloaderCommand, packet, &rspSize, params, &noResponse);
            }
        }
