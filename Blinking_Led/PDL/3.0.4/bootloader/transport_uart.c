...
#define UART_BYTE_TO_BYTE_TIMEOUT_US  (868u)
    ...
    #define CY_BOOTLOAD_UART_INSTANCE       UART
    
    #define UART_API(fn)          UART_API_IMPL(CY_BOOTLOAD_UART_INSTANCE, fn)
    #define UART_API_IMPL(a, b)    UART_API_IMPL2(a, b)
    #define UART_API_IMPL2(a, b)   a ## b

    #define CY_BOOTLOAD_UART_HW    UART_API_IMPL(CY_BOOTLOAD_UART_INSTANCE, _SCB__HW)
    
#else

/* USER CONFIGURABLE: the pointer to the base address of the hardware */
#define CY_BOOTLOAD_UART_HW     ((CySCB_Type *) SCB5)
...
static void UART_Start(void)
{
    ...
    {
        /* Configure component */
        (void) Cy_SCB_UART_Init(CY_BOOTLOAD_UART_HW, &UART_config, &UART_context);

        /* Component is configured */
        UART_initVar = 1U;
    }

    Cy_SCB_UART_Enable(CY_BOOTLOAD_UART_HW);
}

#endif /* defined(CY_PSOC_CREATOR_USED) */

/* Returns a number of bytes to copy into a bootloader buffer */
...
void UART_UartCyBtldrCommStop(void)
{
    ...
    UART_API(_Disable)();
#else
    Cy_SCB_UART_Disable(CY_BOOTLOAD_UART_HW, &UART_context);
#endif /* defined(CY_PSOC_CREATOR_USED) */
}
...
void UART_UartCyBtldrCommReset(void)
{
    ...
    Cy_SCB_UART_ClearRxFifo(CY_BOOTLOAD_UART_HW);
    Cy_SCB_UART_ClearTxFifo(CY_BOOTLOAD_UART_HW);
}
...
* - \ref CY_BOOTLOAD_ERROR_TIMEOUT if no data has been received.
...
cy_en_bootload_status_t UART_UartCyBtldrCommRead(uint8_t pData[], uint32_t size, uint32_t *count, uint32_t timeout)
{
    ...
    if ((pData != NULL) && (size > 0u))
    {
        ...
        {
            /* Check packet start */
            if (Cy_SCB_UART_GetNumInRxFifo(CY_BOOTLOAD_UART_HW) != 0u)
            {
                /* Wait for end of packet */
                do
                {
                    byteCount = Cy_SCB_UART_GetNumInRxFifo(CY_BOOTLOAD_UART_HW);
                    Cy_SysLib_DelayUs(UART_BYTE_TO_BYTE_TIMEOUT_US);
                }
                while (byteCount != Cy_SCB_UART_GetNumInRxFifo(CY_BOOTLOAD_UART_HW));
        ...
        if (status == CY_BOOTLOAD_SUCCESS)
        {
            ...
            (void) Cy_SCB_UART_GetArray(CY_BOOTLOAD_UART_HW, (void*)pData, byteCount);
        }
    }
...
cy_en_bootload_status_t UART_UartCyBtldrCommWrite(uint8_t pData[], uint32_t size, uint32_t *count, uint32_t timeout)
{
    ...
    if ((pData != NULL) && (size > 0u))
    {
        ...
        Cy_SCB_UART_PutArrayBlocking(CY_BOOTLOAD_UART_HW, (void*)pData, size);
